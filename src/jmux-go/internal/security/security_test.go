...
func TestEncryptedConnection(t *testing.T) {
	...
	encConn, err := NewEncryptedConnection(sessionKey)
	if err != nil {
...
func TestSecurityProtocol(t *testing.T) {
	...
	method, err := ParseAuthMessage(authMsg)
	if err != nil {
	...
	challengeMsg := FormatChallengeMessage(nonce)
	
	parsedNonce, err := ParseChallengeMessage(challengeMsg)
	if err != nil {
	...
	responseMsg := FormatResponseMessage(response)
	
	parsedResponse, err := ParseResponseMessage(responseMsg)
	if err != nil {
