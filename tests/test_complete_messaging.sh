...
DMUX_BINARY="/tmp/dmux_complete_test"
...
"$DMUX_BINARY" msg "$(whoami)" "Hello from dmux messaging test!"

sleep 3

echo "3. Testing share command (creates invite messages)..."
timeout 5s "$DMUX_BINARY" share test-session 2>/dev/null || echo "Share command tested"
